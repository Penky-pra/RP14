...
class SimpleMetaPatientBuilder:
    ...
    def extract_encounters(self):
        ...
        encounter_params = {
            "_count": 500,
            "_sort": "-date",
            "subject": "65ba2d4a02c22b64cd57d5c93b91bdd5aac0909e974c580af175920a768a8e88",#patient_id_col,
        }
...
def main():
    ...
    Workflow:
    1. Extract encounters or any resource
    ...
    # Step 3: Merge encounters with meta patients
    merged_df = pd.merge(
        encounters_df,
        meta_patients_df,
        how="inner",
        left_on="patient_id",
        right_on="linked_patient_id",
    )

    merged_df.to_csv(
        builder.output_dir / "encounters_with_meta_patients.csv", index=False
    )

    # Step 4: Do whatever you want with the merged dataframe loop through your meta patients
#ef main():
    """
    Main function to extract encounters, build meta-patients, and merge resources.
    """
    #builder = SimpleMetaPatientBuilder()

    # Define resources to process (Add actual paths)
    resource_files = {
     #   "encounters": "output/encounters.csv",
      #  "procedures": "output/procedures.csv",
       # "observations": "output/observations.csv",
        #"medications": "output/medications.csv",
        #"conditions": "output/conditions.csv",
    }
